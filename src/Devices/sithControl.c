    ...
    if (stdVR_bEnabled && stdVR_IsSessionRunning()) {
        ...
        switch (funcIdx) {
            ...
            case INPUT_FUNC_NEXTWEAPON:
                // Other face button = cycle to next weapon (Y or B)
                vrInput = stdVR_Input_IsButtonPressed(btnNextWeapon);
                break;
            case INPUT_FUNC_GAMESAVE:
                // Movement thumbstick click = quick save
                vrInput = stdVR_Input_IsButtonPressed(btnQuickSave);
                break;
            default:
                break;
        }
